 ...
 */
function encrypt_string_256(string $data, string $key): string
{
 ...
 */
function decrypt_string_256(string $encrypted, string $key)
{
 $key = hash('sha256', $key, true);
...
function set_password(string $vendor, ?string $server, string $username, ?string $password): void
{
  ...
  $_SESSION['encrypted_pwd'] = encrypt_string_256($password, getenv("CIPHER_KEY"));
 }
}
...
{
 if (isset($_SESSION['encrypted_pwd'])) {
  return decrypt_string_256($_SESSION['encrypted_pwd'], getenv("CIPHER_KEY"));
 }
 return false;
}
